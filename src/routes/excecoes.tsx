...
export const Route = createFileRoute("/excecoes")({
  head: () => ({ meta: [{ title: "Exceções · Veridia" }] }),
  component: Excecoes,
});

function Excecoes() {
  const [selected, setSelected] = useState(exceptions[0]);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-warning/10 border border-warning/25 text-[12px] text-warning">
            ...
            17 exceções aguardando revisão
          </div>
        }
      />
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={cn("size-10 rounded-lg grid place-items-center shrink-0",
                    ex.severity === "Alta" ? "bg-destructive/12 text-destructive" :
                    ex.severity === "Média" ? "bg-warning/12 text-warning" : "bg-info/12 text-info"
                  )}>
                    <AlertTriangle className="size-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-mono text-[10.5px] text-muted-foreground">{ex.id}</span>
                      <StatusBadge status={ex.severity} />
                      <span className="text-[11px] text-muted-foreground inline-flex items-center gap-1"><Clock className="size-3" /> {ex.time}</span>
                    </div>
                    <div className="text-[14.5px] font-semibold tracking-tight">{ex.title}</div>
                    <div className="text-[12.5px] text-muted-foreground mt-1">{ex.desc}</div>
                    <div className="mt-3 p-2.5 rounded-md bg-ai/6 border border-ai/15">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-[12.5px] text-foreground/90">{ex.suggestion}</div>
                        </div>
                      </div>
                      <div className="mt-2.5"><ConfidenceBar value={ex.confidence} /></div>
                    </div>
                    <div className="mt-3 flex items-center gap-2">
                      <button className="h-7 px-2.5 rounded-md bg-success/15 text-success border border-success/25 text-[11.5px] font-medium inline-flex items-center gap-1 hover:bg-success/20">
                      ...
                      <button className="h-7 px-2.5 rounded-md border border-border text-[11.5px] text-muted-foreground hover:bg-accent inline-flex items-center gap-1">
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="font-mono text-[10.5px] text-muted-foreground mt-1">{selected.id}</div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <ol className="relative border-l border-border pl-4 space-y-3 ml-1">
                  {[
                    { t: "14:25:03", text: "Entrada DDA Itaú recebida" },
                    { t: "14:25:04", text: "IA tentou match com cadastro de fornecedores" },
                    { t: "14:25:04", text: "Match não encontrado · escalado como exceção" },
                    { t: "14:25:05", text: "Sugestão gerada · aguardando humano" },
                  ].map((e, i) => (
              ...
              <div className="pt-3 border-t border-border flex items-center gap-2">
                <button className="flex-1 h-9 rounded-md bg-foreground text-background text-[12.5px] font-medium inline-flex items-center justify-center gap-1.5 hover:opacity-90">

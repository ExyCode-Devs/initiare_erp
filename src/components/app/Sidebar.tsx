...
function NavIcon({ name, className }: { name: string; className?: string }) {
  const Cmp = (Icons as any)[name] as React.ComponentType<{ className?: string }>;
      ...
      {!collapsed && (
        <div className="px-3 pt-3">
          <button className="w-full flex items-center gap-2.5 rounded-md border border-sidebar-border bg-sidebar-accent/40 px-2.5 py-2 hover:bg-sidebar-accent transition-colors">
            <div className="size-6 rounded bg-gradient-to-br from-chart-4 to-chart-2 grid place-items-center text-[10px] font-semibold text-white">AC</div>
            <div className="text-left min-w-0 flex-1">
              <div className="text-[12px] font-medium truncate">Acme Holdings</div>
              <div className="text-[10px] text-muted-foreground truncate">3 empresas · BR</div>
            </div>
              ...
              {group.items.map((it) => {
                ...
                return (
                  ...
                  >
                    ...
                    <NavIcon name={it.icon} className="size-4 shrink-0" />
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className={cn("flex items-center gap-2.5", collapsed && "justify-center")}>
          <div className="relative">
            <div className="size-8 rounded-full bg-gradient-to-br from-ai to-info grid place-items-center text-[11px] font-semibold text-ai-foreground">RA</div>
            <span className="absolute -bottom-0.5 -right-0.5 size-2.5 rounded-full bg-success ring-2 ring-sidebar" />
          </div>
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-medium truncate">Rafael Almeida</div>
              <div className="text-[10px] text-muted-foreground truncate">CFO · Admin</div>
            </div>
          )}
          {!collapsed && <Icons.MoreHorizontal className="size-4 text-muted-foreground" />}
        </div>
      </div>
    </aside>
  );
